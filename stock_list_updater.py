from src.Constant_Field import DAILY_STOCK_LIST_NAME,DAILY_STOCK_PATH,STOCK_NUMBER_COLUMN,STOCK_NAME_COLUMN,ASSETS_PATH,STOCK_LIST_NAME
from src.Date_Operation import turn_to_datetime
import os
import pandas as pd
daily_stock_list_names = os.listdir(DAILY_STOCK_PATH)
...
latest_daily_stock_df = pd.read_csv(os.path.join(DAILY_STOCK_PATH, latest_daily_stock))
latest_daily_stock_list_df = latest_daily_stock_df[[STOCK_NUMBER_COLUMN, STOCK_NAME_COLUMN]]
latest_daily_stock_list_df.to_csv(os.path.join(ASSETS_PATH,STOCK_LIST_NAME),index = False)
